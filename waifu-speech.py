...
class MicToText:
    ...
    def convert(self):
        try:
            ...
            print(
                f"[Failed] Could not request results from Google Speech Recognition service; {e}")
            return None
...
class TextTranslator:
    ...
    def translate(self, source, target):
        try:
            translated_text = ts.google(self.text, source, target)
            print(f"[Success] Translated: {translated_text}")
            return translated_text
        except Exception as e:
            print(f"[Failed] Translation failed; {e}")
            return None
...
class TextToSpeech:
    ...
    def generate_audio(self, speaker_id=1):
        try:
            ...
            if response.get("success", False):
                audio_url = response.get("mp3DownloadUrl")
                return audio_url
            else:
                return None
        except Exception as e:
    ...
    def save_audio(self, audio_url, file_name):
        try:
            ...
            print(f"[Failed] Audio file saving failed; {e}")
            return False
...
if __name__ == "__main__":
    ...
    if section == 1:
        ...
        text = audio_to_text.convert()
        if not text:
            print("[Failed] Failed to convert audio to text")
            exit()
    else:
        print("Invalid key.")
        exit()
